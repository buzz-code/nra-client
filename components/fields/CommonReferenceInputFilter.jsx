...
export const CommonReferenceInputFilter = ({ label = undefined, source, reference, alwaysOn = {}, dynamicFilter = {}, sort = defaultSortBy }) => {
    const { filterValues } = useListContext();
    const filter = useMemo(() => getDynamicFilter(dynamicFilter, filterValues), [dynamicFilter, filterValues]);

    return (
        <ReferenceInput label={label} source={source} reference={reference} alwaysOn={alwaysOn} filter={filter} sort={sort}>
